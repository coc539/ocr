...
from pyzbar import pyzbar  # Import for barcode detection
...
class WebcamApp:
    def __init__(self, window, window_title):
        ...
        self.sheet.append(["Image Name", "Extracted Text", "Extracted Barcode"])  # Header for the Excel file
            ...
            if ret:
                ...
                for r in results:
                    ...
                    for box in boxes:
                        ...
                        # Perform Tesseract OCR and barcode detection on the saved image
                        extracted_text = self.extract_text_from_image(image_path)
                        extracted_barcodes = self.extract_barcodes_from_image(image_path)
                        print(f"Extracted Text: {extracted_text}")
                        print(f"Extracted Barcodes: {extracted_barcodes}")
                        
                        # Save image name, extracted text, and barcode data to Excel
                        self.save_to_excel(image_path, extracted_text, extracted_barcodes)
    ...
    def extract_barcodes_from_image(self, image_path):
        # Load the image
        ...
        if barcodes:
            ...
            for barcode in barcodes:
                ...
                # Print debugging info about barcode
                print(f"Detected Barcode: {barcode_data}")
                print(f"Type: {barcode_type}")
                print(f"Location: {barcode_rect}")
                
                # Append barcode data
                barcode_data_list.append(barcode_data)
            
            # Join multiple barcode data in a single string, separated by commas
            return ', '.join(barcode_data_list) if barcode_data_list else "No Barcode Detected"
        else:
            print("No barcodes detected.")
            return "No Barcode Detected"

    def save_to_excel(self, image_path, extracted_text, extracted_barcodes):
        # Extract the image filename (without the full path) for the Excel record
        image_name = os.path.basename(image_path)
        # Append the image name, extracted text, and barcode data to the Excel file
        self.sheet.append([image_name, extracted_text, extracted_barcodes])
